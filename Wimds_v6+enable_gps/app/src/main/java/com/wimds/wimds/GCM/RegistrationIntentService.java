    ...
    protected void onHandleIntent(Intent intent) {
        ...
        try {
            synchronized (TAG) {
                ...
                Token.token = instanceID.getToken(default_senderId, scope, null);

                Log.i(TAG, "GCM Registration Token: " + Token.token);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        registrationComplete.putExtra("token", Token.token);
